...
class Element {
  ...
  state() {
    if (!Element.__STATES[this.constructor]) {
      return (Element.__STATES[this.constructor] = this.init_state());
    } else {
  ...
  set_state(value) {
    Element.__STATES[this.constructor] = value;
    this.render_to_dom();
  }
  ...
  id() {
    return this.dom_id || this.__generate_element_id();
  }
  replace_styles() {
    const { sheet } = this.create_style_sheet();
    for (let i = 0; i < sheet.cssRules.length; i++) {
      if (sheet.cssRules[i].selectorText.includes(this.dom_id))
        sheet.deleteRule(i);
    }
