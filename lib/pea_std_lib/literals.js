...
class Record extends Value {
  ...
  __lookup__(key) {
    const value = this.__value.find(([k, v]) => {
  ...
  to_l() {
    return new List(this.__value.map((l) => new List(l)));
  }
  each(fn) {
    this.__value.forEach(([key, value], i) => {
      fn(key, value, i);
    });
  }
  combine(record) {
    let newRecord = this.__value.slice();
