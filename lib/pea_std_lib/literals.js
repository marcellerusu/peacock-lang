...
class Bool extends Value {
  ...
  __and__(other) {
    if (this.to_js() && other.to_b().to_js()) {
      return other;
    } else {
      return this;
    }
...
class Element {
  ...
  render_to_dom() {
    if (
      ...
      this.prev_context.__eq__(this.context().value).to_js()
    ) {
      return;
    }
  ...
  to_dom() {
    let elem = this.view(this.props(), this.state(), this.context().value);
    this.prev_state = this.state();
    this.prev_props = this.props();
    this.prev_context = this.context().value;
