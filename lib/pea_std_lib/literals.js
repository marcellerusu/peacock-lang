...
class List extends Value {
  ...
  to_r() {
    ...
    for (let item of this.__value) {
      const [k, v] = [
        item.__lookup__(Int.create(0)),
        item.__lookup__(Int.create(1)),
      ];
      record[k.__val__()] = v;
    }
  ...
  every(fn) {
    for (let i = 0; i < this.__value.length; i++) {
      const result = fn(this.__value[i], Int.create(i));
      if (!result.__val__()) {
        return new Bool(false);
      }
    }
...
class Component {
  ...
  to_dom() {
    ...
    const class_name = __generate_class_id();
...
const mount_element = (ComponentClass, node) => {
  node.append(new ComponentClass().to_dom());
};
...
const documentBody = globalThis.document && document.body;
