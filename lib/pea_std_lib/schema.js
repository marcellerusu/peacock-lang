...
function matchUnion({ patterns }, value) {
  return patterns.some((p) => match(p, value));
}
...
function match(pattern, value, captures = new Map()) {
  if (pattern instanceof Any) {
    ...
    return matchUnion(pattern, value);
...
function matchArray(patternArr, valueArr, captures = new Map()) {
  if (patternArr.length !== valueArr.length) return false;

  for (let [pattern, value] of zip(patternArr, valueArr)) {
    if (!match(pattern, value, captures)) return false;
  }
  return true;
}
